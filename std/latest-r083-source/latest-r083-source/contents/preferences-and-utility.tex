...
如图（\ref{fig:indefferent-curves_linear-function}），线性效用函数形如$u=f(x,y)=\alpha x + \beta y$，偏导数分别为：
\[
MU_x = f_1=\alpha \text{，} MU_y = f_2=\beta
\]
边际替代率分别为：
\[
MRS_{x~\text{for}~y} = \frac{\alpha}{\beta}
\]
...
\end{tikzpicture}%
\caption{完全互补型偏好}
\label{fig:wanquanhubu-pianhao-a}
\end{minipage}% 
\begin{minipage}[t]{0.5\linewidth} 
\centering
\vspace{0pt}
...
\foreach \num in {5,10,...,20}
{\edef\temp{\noexpand
\addplot[draw=blue,line width=\num/10 pt] coordinates {(\num,25) (\num,\num/4) (7*\num/4,0)};
} \temp }
\node[fill=white,circle,inner sep=0pt,font=\tiny] at (axis cs:5,23) {$U_1$};
\node[fill=white,circle,inner sep=0pt,font=\tiny] at (axis cs:10,23) {$U_2$};
\node[fill=white,circle,inner sep=0pt,font=\tiny] at (axis cs:15,23) {$U_3$};
\node[fill=white,circle,inner sep=0pt,font=\tiny] at (axis cs:20,23) {$U_4$};
\end{axis}
\end{tikzpicture}%
\caption{一个简易变种}
\label{fig:wanquanhubu-pianhao-bianzhong}
\end{minipage} 
\end{shaded*}
\end{figure}
...
如图（\ref{fig:quasilinear-function}），拟线性偏好的效用函数形如$u=f(x,y)=v(x) + y$，其中$v(x)$是递增且严格凹的（$v'>0$，$v''<0$）。整个无差异曲线族可以由其中任何一条曲线沿$y$轴平移得到。拟线性效用函数的偏导数分别为：
\[
